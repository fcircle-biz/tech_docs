...
生成済みの学習教材HTMLを Chrome MCP（`claude-in-chrome`）で実際に開き、**描画結果**を確認して品質をレビューするスキル。HTMLソースを静的に検査する content-reviewer とは相補的で、本スキルは「実際にブラウザで開いた結果」を一次情報とする。

## 使用方法

```
/content-browser-review [対象HTMLパス or ディレクトリ] [scope?]
```
...
例:
- `/content-browser-review docs/guide/cloud-infrastructure/docker`
- `/content-browser-review docs/practice/programming-languages/python-ecosystem/python-basics smoke`
- `/content-browser-review https://fcircle-biz.github.io/tech_docs/guide/.../docker-learning-material-01.html`
...
- content-reviewer: HTMLソースの静的検査（プレースホルダー未置換・スクリプト読込順・命名等）。本スキルは描画結果の確認で相補的。
- 各 creator スキル / content-suite-creator: 生成後の検証フェーズから、静的（content-reviewer）に加えて本スキルで実機確認できる。
