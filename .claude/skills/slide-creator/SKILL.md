...
- テンプレートは `templates/v2/slide/` をコピーして利用する。`main.js` は無編集。
- 単体スキル(並列ファンアウトなし)。準備〜編集〜報告までメイン会話ループが逐次実行する。

## 使用方法

```
/slide-creator [教材タイトル]
```

例:
- `/slide-creator Excel入門`
- `/slide-creator Word基礎`
- `/slide-creator Python入門`
...
- ヘッダー構造は変更しない(テンプレートをそのまま使う)。Mermaid・テキスト視認性ルールはスライドビューアーでは原則使用しないが、本文記述を追加する場合は CLAUDE.md / `templates/v2/reference/mermaid-patterns.md` の方式に従う。
