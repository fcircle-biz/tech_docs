...
```
/tutorial-creator [アプリタイプ] [環境] [DB構成]
```
...
例:
- `/tutorial-creator todo-app-v1 windows環境 postgresqlをdockerで構築`
- `/tutorial-creator user-management-v2 mac環境 sqliteを使用`
...
各サブエージェントへの最小指示（要旨）:
> `.claude/skills/tutorial-creator/references/step3-chapter.md` と共通HTMLルール（同 `references/mermaid-rules.md`・`references/components-and-colors.md`）を読み、第1ステップHTML（パスを渡す）を構造ベースとして、README.md（パスを渡す）から指定ステップ番号の情報を抽出し、1ステップ分のHTMLをWriteせよ。共通部品（`sidebar-content.js`/`styles.css`/`main.js`/`drawing-tool.js`）は上書き禁止。

削除予定の旧agent型（`tutorial-creator-step3` 等）は使わない。

### フェーズ3: 検証

`content-reviewer` スキルの手順（または `references/components-and-colors.md` 末尾のチェックリスト）で、全生成HTMLのテキスト視認性・Mermaid記法・スクリプト読込順・ヘッダー構造・カラー置換漏れを検証する。
