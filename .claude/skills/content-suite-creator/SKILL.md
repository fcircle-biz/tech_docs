---
name: content-suite-creator
description: 1つの技術トピックから複数の資料タイプ(学習ガイド・練習問題・チートシート等)を並列で一括生成し、最後にcontent-reviewerで一括検証するオーケストレーションスキル。1トピックの教材一式をまとめて作成する際に使用する。
---

# Content Suite Creator

1つの技術トピックを起点に、複数の資料タイプ（guide / practice / cheatsheet など）を**並列で一括生成**し、全完了後に content-reviewer で一括検証するオーケストレーションスキル。

## 概要

このスキルは単体で資料を作るのではなく、既存の各 creator スキルと content-reviewer を**束ねて指揮する**役割を持つ（オーケストレーション）。
...
- 委譲先スキルは tech-guide-creator / practice-creator / cheatsheet-creator / tutorial-creator / assignment-creator / slide-creator、検証は content-reviewer。
...
```
/content-suite-creator [技術名] [任意:資料タイプCSV]
```
...
例:
- `/content-suite-creator docker` — 既定セット（guide, practice, cheatsheet）を並列生成
- `/content-suite-creator docker guide,cheatsheet,practice` — 明示指定
- `/content-suite-creator sql-queries guide,cheatsheet` — ガイドとチートシートのみ
...
> `references/material-types.md` の対応表で示された担当資料タイプ（例: guide）の creator スキル（例: tech-guide-creator）の references に従い、技術名「[技術名]」（tutorial/assignment は加えて specs/環境/DB）から1資料タイプ分（README・共通部品・全章/全ステップ/全回HTML）を生成して Write せよ。資料内の第2単位以降は creator スキルの規定どおり並列ファンアウトしてよい。共通部品（JS/CSS/sidebar）は資料タイプ内で重複生成・上書きしないこと。

### 3. 検証（全完了後・一括）

全資料タイプの生成完了後、`content-reviewer` スキルの手順で生成物を**一括検証**する（content-reviewer が未導入の場合は `references/suite-procedure.md` のチェックリストで代替）。検証観点: `{{PRIMARY_*}}` 未置換・スクリプト読込順・Mermaid記法・テキスト視認性・ヘッダー構造・絶対パス混入。
...
- 生成: tech-guide-creator / practice-creator / cheatsheet-creator / tutorial-creator / assignment-creator / slide-creator
- 検証: content-reviewer
