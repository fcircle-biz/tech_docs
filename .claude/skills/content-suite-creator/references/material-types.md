...
content-suite-creator が指揮する各資料タイプの「対応 creator スキル名・必要引数・技術名のみで駆動可能か・出力ディレクトリ・GitHub Pages URL形式」をまとめる。引数解析フェーズ（どのスキルへ何を渡すか・不足情報の確認要否）と統合報告フェーズ（URLの組み立て）でこの表を真実源とする。
...
|-----------|--------------------|----------|----------------------|----------------|
| `guide` | `tech-guide-creator` | 技術名 | 可（技術名のみ） | ○ |
| `practice` | `practice-creator` | 技術名 | 可（技術名のみ） | ○ |
| `cheatsheet` | `cheatsheet-creator` | 技術名＋カテゴリパス | 概ね可（カテゴリパスは技術名から9分類で導出可能） | ○ |
| `tutorial` | `tutorial-creator` | アプリタイプ（specs配下フォルダ名）＋環境＋DB構成 | 不可（specs前提） | ×（明示指定時のみ） |
| `assignment` | `assignment-creator` | 仕様書フォルダパス（specs配下） | 不可（specs前提） | ×（明示指定時のみ） |
| `slide` | `slide-creator` | 教材タイトル（＋事前に `work_pdf/` へPDF配置） | 不可（PDF前提） | ×（明示指定時のみ） |
...
- 駆動可否: 概ね可。カテゴリパスは技術名から9分類体系（`tech-knowledge-map.md`、または cheatsheet-creator の `references/category-taxonomy.md`）で導出する。導出が曖昧な場合のみユーザーに確認する。

### tutorial（チュートリアル）※specs前提

- スキル: `tutorial-creator`
- 引数: `[アプリタイプ] [環境] [DB構成]`
...
- 各 creator スキルにも分類決定用の references がある（例: tech-guide-creator の `references/taxonomy-paths.md`、cheatsheet-creator の `references/category-taxonomy.md`、slide-creator の `references/category-paths.md`）。委譲先スキルへ任せる場合はそれらに従わせる。
