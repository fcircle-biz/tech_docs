...
```
/tech-guide-creator [技術名]
```

例:
- `/tech-guide-creator python-streamlit`
- `/tech-guide-creator docker`
- `/tech-guide-creator spring-boot`
...
2. **共通部品の土台作成** — references/step2-foundation.md に従い、`templates/v2/html/` から4ファイルをコピーし、`styles.css` の `{{PRIMARY_*}}` を実カラー値へ置換、`sidebar-content.js` の `chapters` 配列に全章定義を設定する。
...
- 指示内容: 「`.claude/skills/tech-guide-creator/references/step3-chapter.md` と `.claude/skills/tech-guide-creator/references/html-rules.md` を読み、第1章HTML（フルパスを渡す）を構造ベースとして、README.md（フルパスを渡す）から指定章番号の情報を抽出し、1章分のHTMLを生成して Write せよ。共通部品（`styles.css` / `sidebar-content.js` / `main.js` / `drawing-tool.js`）は上書き禁止。」
- 各呼び出しに渡す可変情報: README.mdフルパス・第1章HTMLフルパス・章番号

### 検証フェーズ

content-reviewer スキルの手順、または references/html-rules.md のチェックリストで全生成物を検証する（`{{PRIMARY_*}}` 未置換・スクリプト読込順・Mermaid記法・テキスト視認性・ヘッダー構造）。
...
- **{{PRIMARY_*}} の置換は必須。** `styles.css` の `{{PRIMARY_400}}` `{{PRIMARY_500}}` `{{PRIMARY_700}}` `{{PRIMARY_RGB}}` を実カラー値へ置換しないとCSSが壊れる。準備フェーズで必ず確認すること（最頻出の破損要因）。
...
- **絶対パスのハードコード禁止。** 作業ルートからの相対パス（`docs/guide/...`、`templates/v2/...`）で扱う。
