...
`templates/v2/html/` から以下4ファイルを技術フォルダにコピーする。
...
**技術別カラー設定一覧**（正典は `templates/v2/reference/color-themes.md`。以下は代表値の抜粋）:
...
| PHP (indigo) | `#818cf8` | `#6366f1` | `#4338ca` | `99, 102, 241` |
| Oracle (red) | `#f87171` | `#ef4444` | `#b91c1c` | `239, 68, 68` |
| AI/ML (purple) | `#c084fc` | `#a855f7` | `#7e22ce` | `168, 85, 247` |
| Git (slate) | `#94a3b8` | `#64748b` | `#334155` | `100, 116, 139` |

**注意**: この変数がヘッダーの背景色に直接影響する。プレースホルダーのまま放置すると CSS エラーになる。
...
`templates/v2/html/learning-material-template.html` をベースに第1章を生成する。

- ファイル名: `[技術名]-learning-material-01.html`
- テンプレート内の `<!-- TODO: ... -->` コメントを検索し編集する。
  - **技術名・タイトル**: プレースホルダーを実際の値に置換。
  - **カラーテーマ**: `tailwind.config` 内の `primary` カラー（`templates/v2/reference/color-themes.md` 参照）。
